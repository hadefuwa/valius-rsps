...
public class RSInterface {
	...
	public static void unpack(Archive streamLoader, TextDrawingArea textDrawingAreas[],
			Archive streamLoader_1, RSFont[] newFontSystem) {
		...
		while (stream.position < stream.payload.length) {
			...
			rsInterface.contentType = stream.readShort();
			...
			if (rsInterface.type == 0) {
				...
				rsInterface.visible = stream.readUnsignedByte() == 1;
			...
			if (rsInterface.type == 4) {
				...
				String interMessage = stream.readString();
				
				if (rsInterface.id == 2451) { // Logout message
					rsInterface.message = interMessage.replaceFirst("RuneScape", Configuration.SHORT_CLIENT_TITLE);
				} else {
					rsInterface.message = interMessage.replaceAll("RuneScape", Configuration.CLIENT_TITLE);
				}
			...
			if (rsInterface.type == 3 || rsInterface.type == 4) {
				rsInterface.secondaryColor = stream.readInt();
				rsInterface.anInt216 = stream.readInt();
			...
			if (rsInterface.type == 6) {
				...
				rsInterface.modelZoom = stream.readShort();
				rsInterface.modelRotation1 = stream.readShort();
				rsInterface.modelRotation2 = stream.readShort();
			}
		...
		Interfaces.autocastFix(streamLoader_1);
		Interfaces.loadInterfaces();
		teleportationInterface2(textDrawingAreas);
		HalloweenDailyReward(textDrawingAreas);
		OsDropViewer(textDrawingAreas);
		achievementPopup2(textDrawingAreas);
		tabbedShop(textDrawingAreas);
		tobSupplies();
		colorPickerTest(textDrawingAreas);
		makeInterface(newFontSystem);
		// aMRUNodes_238 = null;
	}
	public int actualHeight;
	public int actualWidth;
	
	public static void makeInterface(RSFont[] newFontSystem) {
		RSInterface chatBg = addInterface(19004);
		RSInterface invConfig = addInterface(18830);
		invConfig.type = TYPE_INVENTORY;
		invConfig.height = 1;
		invConfig.width = 8;
		invConfig.inv = new int[invConfig.width * invConfig.height];
		invConfig.invStackSizes = new int[invConfig.width * invConfig.height];
		invConfig.invFlags = new int[invConfig.width * invConfig.height];
		invConfig.spritesX = new int[20];
		invConfig.spritesY = new int[20];
		
		RSInterface sizeContainer = addInterface(19003);
		sizeContainer.totalChildren(2);
		sizeContainer.child(0, 18831, 0, 0);
		sizeContainer.child(1, 19031, 0, 0);
		sizeContainer.contentType = 18831;
		
		RSInterface buttonContainer = addInterface(18831);
		RSInterface.setChildren(5, buttonContainer);
		for(int i = 0;i<5;i++) {
			
			RSInterface button = addInterface(18832 + i);
			button.totalChildren(2);
			
			RSInterface background = addHoverButton(18862 + i, "make/makebg", 0, 1, 93, 72, "Select ", -1);
			RSInterface item = addInterface(18872 + i);
			item.type = TYPE_MODEL;
			item.height = 72;
			item.width = 93;
			item.modelType = MEDIA_TYPE_ITEM;
			item.mediaID = 4152;
			item.centerVertically = true;
			
			
			button.child(0, 18862 + i, 0, 0);
			button.child(1, 18872 + i, 0, 0);
			
			buttonContainer.child(i, button.id, 0, 37);
		}
		
		RSInterface buttonSmallContainer = addInterface(19031);
		RSInterface.setChildren(8, buttonSmallContainer);
		for(int i = 0;i<8;i++) {
			
			RSInterface button = addInterface(19032 + i);
			button.totalChildren(2);
			
			RSInterface background = addHoverButton(19062 + i, "make/makebgsmall", 0, 1, 54, 72, "Select ", -1);
			RSInterface item = addInterface(19072 + i);
			item.type = TYPE_MODEL;
			item.height = 72;
			item.width = 54;
			item.modelType = MEDIA_TYPE_ITEM;
			...
			button.child(0, 19062 + i, 0, 0);
			button.child(1, 19072 + i, 0, 0);
			
			buttonSmallContainer.child(i, button.id, 0, 37);
		}
		
		RSInterface amountContainer = addInterface(18838);
		amountContainer.contentType = 18838;
		String[] txt = {"1", "5", "10", "?", "X", "All"};
		RSInterface.setChildren(txt.length, amountContainer);
		for(int i = 0;i<txt.length;i++) {
			RSInterface button = addInterface(18852 + i);
			button.totalChildren(2);
			addToggleButton3(18882 + i, 0, 1, 20000, i + 1, 30, 30, txt[i], "make/makeamt");
			//18882 + i, "makebg", 1, 0, 93, 72, "Select ", -1);
			RSInterface item = addText(18892 + i, txt[i], defaultTextDrawingAreas, 0, 0x43321e, true, false);
			item.width = 30;
			item.height = 30;
			item.centerVertically = true;
			item.anIntArray212 = new int[1];
			item.anIntArray212[0] = i + 1;
			item.anIntArray245 = new int[1];
			item.anIntArray245[0] = 1;
			item.valueIndexArray = new int[1][3];
			item.valueIndexArray[0][0] = 5;
			item.valueIndexArray[0][1] = 20000;
			item.valueIndexArray[0][2] = 0;
			
			item.secondaryColor = 0xFFFFFF;
			item.parentID = button.id;
			
			button.child(0, 18882 + i, 0, 0);
			button.child(1, 18892 + i, 0, 0);
			
			if(txt[i].equals("?")) {
				button.visible = true;
			}
			
			amountContainer.child(i, button.id, i * 30 + (7 * i), 0);
		}

		RSInterface upperText = addText(19000, "What would you like to make?", defaultTextDrawingAreas, 2, 0x43321e, true, false);
		upperText.width = 260;
		RSInterface lowerText = addText(19001, "Choose a quantity, then click an item to begin.", defaultTextDrawingAreas, 0, 0x43321e, true, false);
		lowerText.width = 260;
		RSInterface.setChildren(4, chatBg);
		chatBg.child(0, 19003, 0, 0);
		chatBg.child(1, 18838, 0, 0);
		chatBg.child(2, 19000, 0, 0);
		chatBg.child(3, 19001, 0, 17);
	}
	public boolean reverseRender;
	
	public static void tabbedShop(TextDrawingArea[] tda) {
		int childId = 0;
		int baseX = 8;
		int baseY = 8;//15 x 18 y
		int tabCount = 5;
		int tabYPos = baseY + 45;
		int tabXPos = baseX + 11;
		int tabIdStart = 64150;
		int tabSpriteStart = 64200;
		int tabInvStart = 64250;
		
		RSInterface parent = RSInterface.addInterface(64100);
		String shopDir = "interfaces/shop/";
		RSInterface.addSprite(64101, 0, shopDir + "bg");
		RSInterface.addSprite(64102, 0, shopDir + "invBack");
		addSpriteOnHover(64104, shopDir + "close", 0, 1, 16, 16, "", -1, 3);
		RSInterface.addText(64105, "Shop Title", tda, 2, 0xFFFFFF, true, true, 513, 20);
		parent.totalChildren(5);
		parent.child(childId++, 64101, baseX, baseY);
		parent.child(childId++, 64102, tabXPos, tabYPos + 21);
		parent.child(childId++, 64104, baseX + 477, baseY + 8);
		parent.child(childId++, 64105, baseX, baseY + 9);
		
		RSInterface allTabs = RSInterface.addInterface(64103);
		allTabs.reverseRender = true;
		allTabs.contentType = 64100;
		allTabs.totalChildren(tabCount);
		parent.child(childId++, 64103, 0, 0);
		
		for(int i = 0;i < tabCount;i++) {
			RSInterface tabContainer = RSInterface.addInterface(tabIdStart + i);

			RSInterface tabInventoryContainer = RSInterface.addInterface(tabInvStart + i);
			tabInventoryContainer.totalChildren(1);
			tabInventoryContainer.height = 240;
			tabInventoryContainer.width = 460;
			tabInventoryContainer.scrollMax = 1000;
			tabInventoryContainer.child(0, tabInvStart + tabCount + i, 18, 12);
			
			RSInterface tabInventory = RSInterface.addInterface(tabInvStart + tabCount + i);
			tabInventory.width = 10;
			tabInventory.invSpritePadY = 10;
			tabInventory.invSpritePadX = 11;
			tabInventory.height = 42;
			tabInventory.inv = new int[tabInventory.width * tabInventory.height];
			tabInventory.invStackSizes = new int[tabInventory.width * tabInventory.height];
			tabInventory.invFlags = new int[tabInventory.width * tabInventory.height];
			tabInventory.spritesX = new int[20];
			tabInventory.spritesY = new int[20];
			tabInventory.actions = new String[] {"Value", "Buy 1", "Buy 5", "Buy 10", "Buy X"};
			Arrays.fill(tabInventory.inv, 4152 + (1000 * i));
			Arrays.fill(tabInventory.invStackSizes, 50);
			tabInventory.type = RSInterface.TYPE_INVENTORY;
			
			RSInterface tab = RSInterface.addSpriteOnHover(tabSpriteStart + i, shopDir + "tab", 0, 1, 103, 21, "Select tab", tabSpriteStart + i, 22);
			RSInterface text = RSInterface.addText(tabSpriteStart + 10 + i, "Tab " + (tabSpriteStart + 10 + i), tda, 0, 0xFFFFFF, true, true, 73, 20);
			text.centerVertically = true;
			tab.backgroundSprites = new Sprite[] { imageLoader(2, shopDir + "tab"), tab.sprite1,  tab.sprite2 };  
			tabContainer.totalChildren(3);
			tabContainer.child(0, tabInvStart + i, 0, 25);
			tabContainer.child(1, tabSpriteStart + i, 3 + (92 * i), 0);
			tabContainer.child(2, tabSpriteStart + 10 + i, 16 + (92 * i), 2);

			allTabs.child(i, tabIdStart + i, tabXPos, tabYPos);
	...
	public static void questTab(TextDrawingArea[] tda) {
		...
		try
		{
			...
			aDiary.child(3, 29471, 152, 12);
			aDiary.child(4, 29472, 170, 12);
	...
	public static void addConfigButton_nocache(int id, int parentId, String spriteName, int sprite1, int sprite2, int width, int height, String toolTip,
	...
	public static void addPestControlRewardWidget(TextDrawingArea[] tda) {
		...
		addHoverButton(37004, "interfaces/pestcontrol/reward/image", 3, 21,
				21, "Close", -1, 37005, 3);
		addHoveredButton(37005, "interfaces/pestcontrol/reward/image", 4, 21,
				21, 37006);
		...
		for (int index = 0; index < 35; index += 5) {
			addSprite(37012 + index, imageId,
					"interfaces/pestcontrol/reward/image");
	...
	public static void wellOfGoodWill(TextDrawingArea[] tda) {
		...
		addText(38003, "Experience (1.5x)", tda, 2, 0xFF981F, true, true);
		...
		addInputField(38011, 120, 0xFF981F, "Enter amount", 144, 20, false, false, "[0-9kKmMbB]");
		addText(38012, "0M/50M", tda, 0, 0xFF981F, true, true);
		addText(38013, "0M/30M", tda, 0, 0xFF981F, true, true);
		addText(38014, "0M/75M", tda, 0, 0xFF981F, true, true);
		setChildren(16, wogw);
		...
		addText(38033, "", tda, 0, 0xFF981F, false, true);
		addText(38034, "", tda, 0, 0xFF981F, false, true);
		addText(38035, "", tda, 0, 0xFF981F, false, true);
		addText(38036, "", tda, 0, 0xFF981F, false, true);
		addText(38037, "", tda, 0, 0xFF981F, false, true);
		setChildren(7, last);
	...
	public static void bank(TextDrawingArea[] tda) {
		...
		addClickableSprites(58200, "Deposit Mode: left-click deposits 1 (off) or all (on)", "banktab/07/bank", 5, 6);
		...
		setBounds(58200, 215, 298, 29, rs); // Deposit mode toggle
	}
	...
	/*
	 * Halloween event daily rewards interface
	 * Graphics & Design by: Fat Ghost | Built by: Divine
	 */
	...
	public static void HalloweenDailyReward(TextDrawingArea[] tda) {
		...
		addText(65006, "Missing a day will reset your streak.", tda, 0, 0x188EBA, true, true);
		addSprite(65007, 3, "Interfaces/DailyRewards/MysteryBox");
		addSprite(65008, 1, "Interfaces/DailyRewards/Lamp");
		addSprite(65009, 4, "Interfaces/DailyRewards/CrystalKey");
		addSprite(65010, 2, "Interfaces/DailyRewards/GoldCoin");
		addSprite(65011, 6, "Interfaces/DailyRewards/BloodMoney");
		addSprite(65012, 1, "Interfaces/DailyRewards/Lamp");
		addSprite(65013, 6, "Interfaces/DailyRewards/BloodMoney");
		addSprite(65014, 7, "Interfaces/DailyRewards/Emblem");
		addSprite(65026, 6, "Interfaces/DailyRewards/BloodMoney");
		addSprite(65027, 2, "Interfaces/DailyRewards/GoldCoin");
		addSprite(65028, 5, "Interfaces/DailyRewards/VoteTicket");
		addSprite(65029, 6, "Interfaces/DailyRewards/BloodMoney");
		addSprite(65030, 1, "Interfaces/DailyRewards/Lamp");
		addSprite(65031, 9, "Interfaces/DailyRewards/Pkp");
		addSprite(65032, 6, "Interfaces/DailyRewards/BloodMoney");
		addSprite(65033, 8, "Interfaces/DailyRewards/LavaPhat");
		addText(65015, "Day 1", tda, 0, 0x188EBA, true, true);
		addText(65016, "Day 2", tda, 0, 0x188EBA, true, true);
		addText(65017, "Day 3", tda, 0, 0x188EBA, true, true);
		addText(65018, "Day 4", tda, 0, 0x188EBA, true, true);
		addText(65019, "Day 5", tda, 0, 0x188EBA, true, true);
		addText(65020, "Day 6", tda, 0, 0x188EBA, true, true);
		addText(65021, "Day 7", tda, 0, 0x188EBA, true, true);
		addText(65022, "Day 8", tda, 0, 0x188EBA, true, true);
		addText(65034, "Day 9", tda, 0, 0x188EBA, true, true);
		addText(65035, "Day 10", tda, 0, 0x188EBA, true, true);
		addText(65036, "Day 11", tda, 0, 0x188EBA, true, true);
		addText(65037, "Day 12", tda, 0, 0x188EBA, true, true);
		addText(65038, "Day 13", tda, 0, 0x188EBA, true, true);
		addText(65039, "Day 14", tda, 0, 0x188EBA, true, true);
		addText(65040, "Day 15", tda, 0, 0x188EBA, true, true);
		addText(65041, "Day 16", tda, 0, 0x188EBA, true, true);
		addText(65023, "", tda, 0, 0xC4C10F, true, true);
		addText(65048, "", tda, 0, 0xC4C10F, true, true);
		addText(65042, "Daily Bonus:", tda, 2, 0x188EBA, false, true);//curret daily bonus
		addText(65043, "Bonus XP:", tda, 2, 0x188EBA, false, true);//bonus xp
		addText(65044, "Double Droprate:", tda, 2, 0x188EBA, false, true);//Bonus Drops
		addText(65045, "Event Boss:", tda, 2, 0x188EBA, false, true);
		addText(65046, "Bonus Vote Points:", tda, 2, 0x188EBA, false, true);
		addText(65047, "Players Online:", tda, 2, 0x188EBA, false, true);
		addText(65048, "1", tda, 2, 0x188EBA, false, true);//Daily bonus
		addText(65049, "2", tda, 2, 0x188EBA, false, true);//Bonus XP
		addText(65050, "3", tda, 2, 0x188EBA, false, true);//Double drop rate
		addText(65051, "4", tda, 2, 0x188EBA, false, true);//Event boss
		addText(65052, "5", tda, 2, 0x188EBA, false, true);//Bonus vote points
		addText(65053, "6", tda, 2, 0x188EBA, false, true);//Players online count
		addText(65054, "Well of Goodwill", tda, 2, 0x188EBA, true, true);
		addText(65055, "This is some text", tda, 2, 0x188EBA, true, true);//WOGW

		interfaceCache[65054].width = 245;
		interfaceCache[65055].width = 245;
		
        RSInterface scroller = addTabInterface(65024);
		scroller.height = 90;
		scroller.width = 180;
		scroller.scrollMax = 350;
		
		setChildren(32, scroller);
		setChildren(22, rsI);
		setBounds(65007, 1, 55, 0, scroller);//mbox 1
		setBounds(65008, 48, 17, 1, scroller);//lamp 1
		setBounds(65009, 89, 13, 2, scroller);//ckey 1
		setBounds(65010, 135, 12, 3, scroller);//gold coin 1
		setBounds(65011, 1, 12, 4, scroller);//bloodmoney 1
		setBounds(65012, 48, 61, 5, scroller);//lamp 2
		setBounds(65013, 89, 55, 6, scroller);//bloodmoney 2
		setBounds(65014, 140, 54, 7, scroller);//emblem 1
		setBounds(65015, 13, 1, 8, scroller);//day 1
		setBounds(65016, 60, 1, 9, scroller);//day 2
		setBounds(65017, 105, 1, 10, scroller);//day 3
		setBounds(65018, 150, 1, 11, scroller);//day 4
		setBounds(65019, 13, 41, 12, scroller);//day 5
		setBounds(65020, 60, 41, 13, scroller);//day 6
		setBounds(65021, 105, 41, 14, scroller);//day 7
		setBounds(65022, 150, 41, 15, scroller);//day 8
		setBounds(65034, 13, 85, 16, scroller);//day 9
		setBounds(65035, 60, 85, 17, scroller);//day 10
		setBounds(65036, 105, 85, 18, scroller);//day 11
		setBounds(65037, 150, 85, 19, scroller);//day 12
		setBounds(65038, 16, 125, 20, scroller);//day 13
		setBounds(65039, 60, 125, 21, scroller);//day 14
		setBounds(65040, 105, 125, 22, scroller);//day 15
		setBounds(65041, 150, 125, 23, scroller);//day 16
		setBounds(65026, 1, 97, 24, scroller);//bloodmoney 3
		setBounds(65027, 48, 95, 25, scroller);//gold coin 2
		setBounds(65028, 89, 97, 26, scroller);//vote tickets
		setBounds(65029, 132, 97, 27, scroller);//bloodmoney 4
		setBounds(65030, 1, 145, 28, scroller);//lamp 3
		setBounds(65031, 48, 137, 29, scroller);//pkp 1
		setBounds(65032, 89, 137, 30, scroller);//bloodmoney 5
		setBounds(65033, 128, 130, 31, scroller);//lava phat
		setBounds(65001, 4, 30, 0, rsI);//text
		setBounds(65002, 250, 38, 1, rsI);
		setBounds(65003, 350, 294, 2, rsI);
		setBounds(65004, 395, 102, 3, rsI);
		setBounds(65005, 387, 241, 4, rsI);
		setBounds(65006, 395, 262, 5, rsI);
        setBounds(65023, 396, 86, 6, rsI);
        setBounds(65024, 300, 146, 7, rsI);//scroll bar
        setBounds(65042, 32, 87, 8, rsI);//current daily bonus
        setBounds(65043, 32, 107, 9, rsI);//bonus xp
        setBounds(65044, 32, 127, 10, rsI);//droprate
        setBounds(65045, 32, 147, 11, rsI);//event boss
        setBounds(65046, 32, 167, 12, rsI);//Double vote points
        setBounds(65047, 32, 299, 13, rsI);//Players online:
        setBounds(65048, 114, 87, 14, rsI);//On/Off toggles daily 
        setBounds(65049, 99, 107, 15, rsI);//On/Off toggles bonus xp
        setBounds(65050, 144, 127, 16, rsI);//On/Off toggles droprate
        setBounds(65051, 112, 147, 17, rsI);//On/Off toggles event boss
        setBounds(65052, 158, 167, 18, rsI);//On/Off toggles double vote
        setBounds(65053, 137, 299, 19, rsI);//player count
        setBounds(65054, 23, 242, 20, rsI);//Well Of Good Will:
        setBounds(65055, 23, 264, 21, rsI);
	}
	
	/*
	 * The first/main page of the NEW teleporting interface 
	 * Graphics & Design by: Fat Ghost | Puzzle Builder: Divine 
	 */
	...
	public static void teleportationInterface2(TextDrawingArea[] tda) {
		...
		addClickableText(63055, "Monsters", "Monster & Slayer Teleports", tda, 0, 0xFF981F, true, true, buttonWidth, buttonHeight);
		addClickableText(63056, "Bosses", "Boss Teleports", tda, 0, 0xFF981F, true, true, buttonWidth, buttonHeight);
		addClickableText(63057, "Minigames", "Minigame Teleports", tda, 0, 0xFF981F, true, true, buttonWidth, buttonHeight);
		addClickableText(63058, "Skilling", "Skilling Teleports", tda, 0, 0xFF981F, true, true, buttonWidth, buttonHeight);
		addClickableText(63059, "Wilderness", "Wilderness Teleports", tda, 0, 0xFF981F, true, true, buttonWidth, buttonHeight);
		addClickableText(63060, "Cities", "City & Donator Teleports", tda, 0, 0xFF981F, true, true, buttonWidth, buttonHeight);
		
		addClickableText(63120, "Select Teleport", "Teleport to selected location", tda, 0, 0xFF981F, true, true, 160, 25);//confirm text
		...
		inventoryWidget.inv = new int[inventoryWidget.width * inventoryWidget.height];
		inventoryWidget.invStackSizes = new int[inventoryWidget.width * inventoryWidget.height];
		...
		RSInterface npcWidget = RSInterface.addInterface(63116);
		npcWidget.type = 6;
		npcWidget.width = 140;
		...
		for(int i = 0;i<buttonCount;i++) {
			...
			RSInterface text = addClickableText(startingTextId, "", "", tda, 0, 0xFF981F, true, true, clickableButtonWidth, 35);
			text.centerVertically = true;
			...
			text = addClickableText(startingTextId, "", "", tda, 0, 0xFF981F, true, true, clickableButtonWidth, 35);
			text.centerVertically = true;
			text.underSpriteWidget = startingId;
		...
		setBounds(63116, itemListX, 70, 11, rsInterface);//Loot rewards text
	...
	public static void drawBlackBox(int xPos, int yPos) {
		...
		Raster.drawPixels(71, yPos - 1, xPos - 2, 0x726451, 1); // Left
																		// line
		Raster.drawPixels(69, yPos, xPos + 174, 0x726451, 1); // Right line
		Raster.drawPixels(1, yPos - 2, xPos - 2, 0x726451, 178); // Top
																		// Line
		Raster.drawPixels(1, yPos + 68, xPos, 0x726451, 174); // Bottom
																	// Line

		// /Dark Coloured Borders\\\
		Raster.drawPixels(71, yPos - 1, xPos - 1, 0x2E2B23, 1); // Left
																		// line
		Raster.drawPixels(71, yPos - 1, xPos + 175, 0x2E2B23, 1); // Right
																		// line
		Raster.drawPixels(1, yPos - 1, xPos, 0x2E2B23, 175); // Top line
		Raster.drawPixels(1, yPos + 69, xPos, 0x2E2B23, 175); // Top line

		// /Black Box\\\
		Raster.method335(0x000000, yPos, 174, 68, 220, xPos); // Yes
																	// method335
																	// is
																	// galkons
																	// opacity
	...
	public static void addToggleButton3(int id, int sprite1, int sprite2, int setconfig, int reqVal, int width, int height, String tooltip, String spriteName) {
		RSInterface rsi = addInterface(id);
		rsi.sprite1 = imageLoader(sprite1, spriteName);
		rsi.sprite2 = imageLoader(sprite2, spriteName);
		rsi.anIntArray212 = new int[1];
		rsi.anIntArray212[0] = reqVal;
		rsi.anIntArray245 = new int[1];
		rsi.anIntArray245[0] = 1;
		rsi.valueIndexArray = new int[1][3];
		rsi.valueIndexArray[0][0] = 5;
	...
	public static void addEquipmentSprite(int id, int firstSprite, int secondSprite) {
		RSInterface rsinterface = interfaceCache[id] = new RSInterface();
		rsinterface.id = id;
		rsinterface.parentID = id;
		rsinterface.type = 5;
		...
		rsinterface.aByte254 = 0;
		rsinterface.mOverInterToTrigger = 52;
		rsinterface.sprite1 = imageLoader(firstSprite, "Equipment/sprite");
		rsinterface.sprite2 = imageLoader(secondSprite, "Equipment/sprite");
	}
	...
	public static RSInterface addText(int id, String text, TextDrawingArea tda[], int idx, int color, boolean center,
			boolean shadow) {
		return addText(id, text, tda, idx, color, center, shadow, 0, 11);
	}
	public static RSInterface addText(int id, String text, TextDrawingArea tda[], int idx, int color, boolean center,
			boolean shadow, int width, int height) {
		RSInterface tab = addTabInterface(id);
		tab.parentID = id;
		tab.id = id;
		tab.type = 4;
	...
	public static RSInterface addHoverButton(int interfaceId, String imageName, int hoveredId, int unhoveredId, int width, int height, String text,
			int contentType) {// hoverable button
		...
		tab.contentType = contentType;
		tab.aByte254 = 0;
		tab.sprite1 = imageLoader(unhoveredId, imageName);
		tab.sprite2 = imageLoader(hoveredId, imageName);
		tab.width = width;
		tab.height = height;
		tab.tooltip = text;
		tab.spriteOpacity = 255;
	...
	public static void addHoverSprite(int id, String imageName, int unhoveredSprite, int hoverOverSprite, int width, int height, String text,
			int contentType, int aT) {// hoverable button
		RSInterface tab = addTabInterface(id);
		tab.id = id;
		tab.parentID = id;
		tab.type = 5;
		...
		tab.contentType = contentType;
		tab.aByte254 = 0;
		tab.sprite1 = imageLoader(unhoveredSprite, imageName);
		tab.sprite2 = imageLoader(hoverOverSprite, imageName);
		tab.width = width;
		tab.height = height;
		tab.tooltip = text;
	...
	public Model method206(int i, int j, int x, int y, boolean refresh) {
		Model model = refresh ? null : (Model) aMRUNodes_264.insertFromCache((i << 16) + j);
		if (model != null)
			return model;
		if (i == MEDIA_TYPE_MODEL)
			model = Model.method462(j);
		else if (i == MEDIA_TYPE_NPC_CHAT)
			model = NpcDefinition.forID(j).method160();
		else if (i == MEDIA_TYPE_MYPLAYER)
			model = Client.myPlayer.method453();
		else if (i == MEDIA_TYPE_ITEM)
			model = ItemDefinition.forID(j).method202(50);
		else if (i == MEDIA_TYPE_CACHED_MODEL)
			model = (Model) aMRUNodes_264.insertFromCache((i << 16) + j);
		else if(i == MEDIA_TYPE_NPC_WORLD) {
			NpcDefinition def = NpcDefinition.forID(j);
			model = def.method164(x, y, null);
		} else if(i == MEDIA_TYPE_ITEM_WORLD) {
			model = ItemDefinition.forID(j).method196(Client.myPlayer.gender);
		}
		if (model != null)
	...
	protected static Sprite method207(int i, Archive streamLoader, String s) {
	...
	public Model method209(int j, int k, boolean flag, boolean refresh) {
		Model model;
		if (flag)
			model = method206(anInt255, anInt256, j, k, refresh);
		else
			model = method206(modelType, mediaID, j, k, refresh);
		if (model == null)
			return null;
		if (k == -1 && j == -1 && model.face_color == null)
			return model;
		Model model_1 = new Model(modelType != 7, Frame.method532(k) & Frame.method532(j), false, model);
		if (k != -1 || j != -1)
			model_1.method469();
		if (k != -1)
			model_1.method470(k);
	...
	public void cacheModel(Model model, boolean flag) {
		if(model == null)
			return;
		int i = modelType;
		int j = mediaID;
	...
	public static RSInterface addClickableText(int id, String text, String tooltip, TextDrawingArea tda[], int idx, int color,
			boolean center, boolean shadow, int width) {
		return addClickableText(id, text, tooltip, tda, idx, color, center, shadow, width, 11);
	}
	...
	public static void tobSupplies() {
		RSInterface bg = RSInterface.addInterface(50000);
		RSInterface.addSprite(50001, 0, "interfaces/tob/bg");
		int[] items = {113, 2428, 2430,2432};
		String[] costs = {"1", "2", "3", "3",};
		
		RSInterface tabInventory = RSInterface.addInterface(50002);
		tabInventory.height = 2;
		tabInventory.width = 5;
		tabInventory.invSpritePadY = 20;
		tabInventory.invSpritePadX = 20;
		tabInventory.inv = new int[tabInventory.width * tabInventory.height];
		tabInventory.invLabels = new String[tabInventory.width * tabInventory.height];;
		tabInventory.invStackSizes = new int[tabInventory.width * tabInventory.height];
		tabInventory.spritesX = new int[20];
		tabInventory.spritesY = new int[20];
		tabInventory.actions = new String[] {"Buy 1", "Buy 5", "Buy 10", null, null};
		for(int index = 0;index<items.length;index++) {
			tabInventory.inv[index] = items[index] + 1;
			tabInventory.invLabels[index] = costs[index];
		}
		Arrays.fill(tabInventory.invStackSizes, 1);
		
		tabInventory.type = RSInterface.TYPE_INVENTORY_WITH_LABELS;
		RSInterface.setChildren(2, bg);
		bg.child(0, 50001, 19, 30);
		bg.child(1, 50002, 80, 80);
	}
	...
	public static void colorPickerTest(TextDrawingArea[] tda) {
		int baseId = 59000;
		RSInterface rsItf = addInterface(baseId);

		addSpriteOnHover(baseId + 102, "interfaces/cape/close", 0, 1, 16, 16, "", -1, 3);
		addSprite(baseId + 101, 0, "interfaces/cape/bg");
		int pickerBaseX = 210;
		int pickerBaseY = 82;
		int[][] positions = {{pickerBaseX, pickerBaseY}, {pickerBaseX + 100, pickerBaseY}, {pickerBaseX, pickerBaseY + 100}, {pickerBaseX + 100, pickerBaseY + 100}};
		String[] text = {"Detail (top)", "BG (top)", "Detail (bottom)", "BG (bottom)"};
		RSInterface modelView = addInterface(baseId + 100);
		modelView.type = TYPE_MODEL_COLOUR_CHANGER;
		modelView.width = 155;
		modelView.height = 250;
		modelView.colourFind = new int[] {65214, 65200, 65186, 62995};
		int colourCount = modelView.colourFind.length;
		int presetCount = 3;
		int indexOffset = 8;
		setChildren((colourCount * 2) + presetCount + indexOffset, rsItf);
		modelView.colourReplace = new int[colourCount];
		...
		modelView.modelType = 7;
		modelView.mediaID = 33019;		
		modelView.anInt257 = -1;

		ItemDefinition itemDef = ItemDefinition.forID(modelView.mediaID);
		modelView.modelRotation1 = itemDef.modelRotation1;
		modelView.modelRotation2 = itemDef.modelRotation2;
		modelView.modelZoom = 420;

		addText(baseId + 103, "Presets", defaultTextDrawingAreas, 2, 0xe88300, true, true);
		addText(baseId + 104, "Select a colour to edit:", defaultTextDrawingAreas, 2, 0xe88300, true, true);
		addText(baseId + 105, "Skillcape Customiser", defaultTextDrawingAreas, 2, 0xe8bf00, true, true);
		
		RSInterface button = addSpriteOnHover(baseId + 106, "interfaces/cape/done", 0, 1, 106, 26, "Done", -1, 1);
		RSInterface doneTxt = addText(baseId + 107, "Apply", defaultTextDrawingAreas, 2, 0xFFFFFF, true, true);
		doneTxt.width = 106;
		doneTxt.height = 23;
		doneTxt.centerVertically = true;
		
		rsItf.child(0, baseId + 101, 17, 20);
		rsItf.child(1, baseId + 102, 480, 26);
		rsItf.child(2, modelView.id, 108, 95);
		rsItf.child(3, baseId + 103, 451, 54);
		rsItf.child(4, baseId + 104, 301, 54);
		rsItf.child(5, baseId + 105, 271, 26);
		rsItf.child(6, baseId + 106, 253, 282);
		rsItf.child(7, baseId + 107, 253, 282);
		

		ColorPickerGroup cpg = ColorPickerGroup.bind(rsItf);
		rsItf.colorPickerGroup = cpg;
		button.colorPickerGroup = cpg;
		for(int index = 0;index<presetCount;index++) {
			int childId = baseId + (colourCount * 2) + (index) + 1;
			RSInterface child = addInterface(childId);
			child.type = TYPE_COLOR_PICKER_PRESET;
			ColorPickerPreset preset = new ColorPickerPreset(index, cpg, modelView.colourFind);
			child.preset = preset;
			
			rsItf.child(colourCount + index + indexOffset, childId, 413, 77 + (78 * index));
		}
	...
	public void generateDisplayModel() {
		try {
			Model newModel = method206(modelType, mediaID, -1, -1, true);
			
			if(newModel == null) {
				return;
			}
			Model model_1 = new Model(modelType != 7, false, false, newModel);
			IntStream.of(model_1.face_color).distinct().forEach(System.out::println);
			for(int colourIndex = 0;colourIndex < colourFind.length;colourIndex++) {
				model_1.method476(colourFind[colourIndex], colourReplace[colourIndex]);
				System.out.println("Replaced " + colourFind[colourIndex] + " with " + colourReplace[colourIndex]);
			}
	
			model_1.method479(64, 768, -10, -50, -10, true);
			displayModel = model_1;
