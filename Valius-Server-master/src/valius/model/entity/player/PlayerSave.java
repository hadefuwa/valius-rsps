...
import valius.backup.BackupHandler.BackupType;
import valius.content.Tutorial.Stage;
import valius.content.achievement_diary.DifficultyAchievementDiary.EntryDifficulty;
...
import valius.content.presets.PresetContainer;
import valius.content.titles.Title;
...
import valius.model.holiday.christmas.Christmas;
...
public class PlayerSave {
	...
	public static boolean playerExists(String name) {
		File file = new File(Config.CHARACTER_SAVE_DIRECTORY + name + ".txt");
		return file.exists();
	}
	...
	public static void addItemToFile(String name, int itemId, int itemAmount) {
		...
		File character = new File(Config.CHARACTER_SAVE_DIRECTORY + name + ".txt");
	...
	public static long[] getFriends(String name) {
		...
		int totalFriends = 0;
		int index = 0;
		File input = new File(Config.CHARACTER_SAVE_DIRECTORY + name + ".txt");
		if (!input.exists()) {
			return null;
		}
	...
	public static Player loadGame(String playerName) {
		Player p = new Player(0, playerName, null);
		String line = "";
		String token = "";
		String token2 = "";
		String[] token3 = new String[3];
		...
		boolean File1 = false;
		try {
			characterfile = new BufferedReader(new FileReader(Config.CHARACTER_SAVE_DIRECTORY + playerName + ".txt"));
			File1 = true;
			...
			if (spot > -1) {
				...
				case 2:
					if (token.equals("character-height")) {
						...
						p.setMacAddress(token2);
					} else if (token.equals("character-ip-address")) {
						p.setIpAddress(token2);
						...
						p.setX((Integer.parseInt(token2) <= 0 ? 3210 : Integer.parseInt(token2)));
					} else if (token.equals("character-posy")) {
						p.setY((Integer.parseInt(token2) <= 0 ? 3424 : Integer.parseInt(token2)));
					...
					} else if (token.equals("bonusXpTime25")) {
						p.bonusXpTime25 = Integer.parseInt(token2);
					} else if (token.equals("bonusXpTime50")) {
						p.bonusXpTime50 = Integer.parseInt(token2);
					} else if (token.equals("bonusXpTime75")) {
						p.bonusXpTime75 = Integer.parseInt(token2);
					} else if (token.equals("bonusXpTime100")) {
						p.bonusXpTime100 = Integer.parseInt(token2);
					} else if (token.equals("bonusXpTime150")) {
						p.bonusXpTime150 = Integer.parseInt(token2);
					} else if (token.equals("bonusXpTime200")) {
						p.bonusXpTime200 = Integer.parseInt(token2);
					} else if (token.equals("vorkathKillCount")) {
						p.vorkathKillCount = Integer.parseInt(token2);
					...
					} else if (token.equals("new-crystal-bow-shots")) {
						p.cBowArrowCount = Integer.parseInt(token2);
					} else if (token.equals("new-crystal-hally-count")) {
						p.cHallyCount = Integer.parseInt(token2);
					} else if (token.equals("new-crystal-shield-count")) {
						p.cShieldCount = Integer.parseInt(token2);
					} else if (token.equals("new-crystal-saeldor-count")) {
						p.saeldorCount = Integer.parseInt(token2);
					} else if (token.equals("seren-Charge")) {
						p.serenCharge = Integer.parseInt(token2);
					} else if (token.equals("sirenic-Mask-Charge")) {
						p.sirenicMaskCharge = Integer.parseInt(token2);
					} else if (token.equals("sirenic-Body-Charge")) {
						p.sirenicBodyCharge = Integer.parseInt(token2);
					} else if (token.equals("sirenic-Chaps-Charge")) {
						p.sirenicChapsCharge = Integer.parseInt(token2);
					} else if (token.equals("ethereum-Charge")) {
						p.ethereumCharge = Integer.parseInt(token2);
					} else if (token.equals("craw-Charge")) {
						p.crawCharge = Integer.parseInt(token2);
					} else if (token.equals("thammaron-Charge")) {
						p.thammaronCharge = Integer.parseInt(token2);
					} else if (token.equals("viggora-Charge")) {
					...
					} else if (token.equals("ice-wyrm-technique")) {
						p.getSlayer().setIceWyrmTechnique(Boolean.parseBoolean(token2));
					...
					} else if (token.equals("pvmPoints")) {
						p.pvmPoints = Integer.parseInt(token2);
					...
					}   else if (token.equals("lastDayClaimed")) {
						p.lastDayClaimed = Integer.parseInt(token2);
					} else if (token.equals("dailyRewardCombo")) {
						p.dailyRewardCombo = Integer.parseInt(token2);
					}
					...
					} else if (token.equals("total-xeric")) {
						p.totalXericFinished = Integer.parseInt(token2);
					} else if (token.equals("total-theatre")) {
						p.totalTheatreFinished = Integer.parseInt(token2);
						...
						p.getCompletionistCape().setColours(colours[0], colours[1], colours[2], colours[3]);
						...
						if (token3.length > 1) {
							for (int i = 0; i < token3.length; i += 2) {
								...
								p.getZulrahLostItems().add(new Item(itemId, itemAmount));
							}
						}
						...
						if (token3.length > 1) {
							for (int i = 0; i < token3.length; i += 2) {
								int itemId = Integer.parseInt(token3[i]);
								int itemAmount = Integer.parseInt(token3[i + 1]);
								p.getCerberusLostItems().add(new Item(itemId, itemAmount));
							}
						}
						...
						if (token3.length > 1) {
							for (int i = 0; i < token3.length; i += 2) {
								int itemId = Integer.parseInt(token3[i]);
								int itemAmount = Integer.parseInt(token3[i + 1]);
								p.getSkotizoLostItems().add(new Item(itemId, itemAmount));
							}
						}
					}
					break;
				...
				case 5:
					if (token.equals("character-skill")) {
						...
						if (token3.length > 3) {
							p.skillLock[skill.getId()] = Boolean.parseBoolean(token3[3]);
							p.prestigeLevel[skill.getId()] = Integer.parseInt(token3[4]);
						}
					}
					break;
				case 6:
				...
				case 46:
					if (token.equals("bag-item")) {
						...
						p.getLootingBag().items.add(new Item(id, amt));
					}
					break;
				case 52:
				...
				case 58:
					if (token.equals("safebox-item")) {
						int id = Integer.parseInt(token3[1]);
						int amt = Integer.parseInt(token3[2]);
						p.getSafeBox().items.add(new Item(id, amt));
					}
					break;
				case 7:
						...
						if (token3.length > 2) {
							...
							for (int i = 0; i < token3.length; i += 3) {
								...
								if (token.startsWith("Inventory")) {
									p.getPresets().getPresets().get(presetId).getInventory().getItems().put(slot, new Item(itemId, amount));
								} else {
									p.getPresets().getPresets().get(presetId).getEquipment().getItems().put(slot, new Item(itemId, amount));
								}
							}
						}
					}
				...
				if (line.equals("[ACCOUNT]")) {
					...
					log.info("Loaded player: {}", p.playerName);
					return p;
				}
			}
			try {
			...
			if (spot > -1) {
				...
				case 2:
					if (token.equals("character-height")) {
						...
						p.getRights().setPrimary(Right.get(Integer.parseInt(token2)));
					} else if (token.equals("group-ironman-leader")) {
						p.getGroupIronman().setLeader(token2);
//					} else if (token.equals("group-ironman-teamates")) {
//						Arrays.stream(token3).forEach(name -> p.getGroupIronman().getTeamates().add(name));
					} else if (token.equals("character-rights-secondary")) { // sound like an activist group
						Arrays.stream(token3).forEach(right -> p.getRights().add(Right.get(Integer.parseInt(right))));
					} else if (token.equals("revert-option")) {
						p.setRevertOption(token2);
						...
						p.bonusXpTime200 = Integer.parseInt(token2);
					} else if (token.equals("has-gauntlet-loot")) {
						p.setGaunletLootAvailable(Boolean.parseBoolean(token2));
					} else if (token.equals("vorkathKillCount")) {
						p.vorkathKillCount = Integer.parseInt(token2);
					} else if (token.equals("raidPoints")) {
						p.setRaidPoints(Integer.parseInt(token2));
						...
						p.getSlayer().setTaskAmount(Integer.parseInt(token2));
						if(p.getSlayer().getTaskAmount() <= 0) {
							p.getSlayer().setTask(Optional.empty());
							p.getSlayer().setTaskAmount(0);
						}
					} else if (token.equals("consecutive-tasks")) {
						p.getSlayer().setConsecutiveTasks(Integer.parseInt(token2));
					} else if (token.equals("mage-arena-points")) {
	...
	//@SuppressWarnings("resource")
	public static boolean saveGame(Player p, boolean inWorld) {
		if(inWorld)
			if (!p.saveFile || p.newPlayer || !p.saveCharacter) {
				// System.out.println("first");
				return false;
			}
		if (p.playerName == null || (inWorld && PlayerHandler.players[p.getIndex()] == null)) {
			// System.out.println("second");
			return false;
		}
		p.getQuestManager().save();
		World.getWorld().getHiscores().prepare(p);
		...
		try {
			File saveDir = new File(Config.CHARACTER_SAVE_DIRECTORY);
			if(!saveDir.exists())
				saveDir.mkdirs();
			File file = new File(Config.CHARACTER_SAVE_DIRECTORY + p.playerName + ".txt");
			BackupHandler.requestBackup(BackupType.PLAYER, file);
			characterfile = new BufferedWriter(new FileWriter(file));
			...
			String passToWrite = inWorld ? Misc.md5Hash(p.playerPass) : p.playerPass;
			...
			characterfile.newLine();
			characterfile.write("has-gauntlet-loot = " + p.isGaunletLootAvailable());
			characterfile.newLine();
			...
			characterfile.write(Integer.toString(p.getHeight()), 0, Integer.toString(p.getHeight()).length());
			characterfile.newLine();
			...
			characterfile.write(Integer.toString(p.getX()), 0, Integer.toString(p.getX()).length());
			characterfile.newLine();
			characterfile.write("character-posy = ", 0, 17);
			characterfile.write(Integer.toString(p.getY()), 0, Integer.toString(p.getY()).length());
			characterfile.newLine();
			...
			characterfile.newLine();
			characterfile.write("bonusXp25 = " + p.bonusXpTime25);
			characterfile.newLine();
			characterfile.write("bonusXp50 = " + p.bonusXpTime50);
			characterfile.newLine();
			characterfile.write("bonusXp75 = " + p.bonusXpTime75);
			characterfile.newLine();
			characterfile.write("bonusXp100 = " + p.bonusXpTime100);
			characterfile.newLine();
			characterfile.write("bonusXp150 = " + p.bonusXpTime150);
			characterfile.newLine();
			characterfile.write("bonusXp200 = " + p.bonusXpTime200);
			characterfile.newLine();
			characterfile.write("VorkathKC = " + p.vorkathKillCount);
			characterfile.newLine();
			...
			characterfile.newLine();
			if (p.getMode().isGroupIronman()) {
				if (p.getGroupIronman().getTeamates().size() > 1) {
					characterfile.write("group-ironman-leader = " + p.getGroupIronman().getLeader());
					characterfile.newLine();
//					characterfile.write("group-ironman-teamates = ");
//					for (String name : p.getGroupIronman().getTeamates()) {
...
//					characterfile.newLine();
			...
			characterfile.newLine();
			characterfile.write("ice-wyrm-technique = " + p.getSlayer().isIceWyrmTechnique());
			characterfile.newLine();
			...
			characterfile.newLine();
			characterfile.write("new-crystal-bow-shots = ", 0, 24);
			characterfile.write(Integer.toString(p.cBowArrowCount), 0, Integer.toString(p.cBowArrowCount).length());
			characterfile.newLine();
			characterfile.write("new-crystal-hally-count = ", 0, 26);
			characterfile.write(Integer.toString(p.cHallyCount), 0, Integer.toString(p.cHallyCount).length());
			characterfile.newLine();
			characterfile.write("new-crystal-shield-count = ", 0, 27);
			characterfile.write(Integer.toString(p.cShieldCount), 0, Integer.toString(p.cShieldCount).length());
			characterfile.newLine();
			characterfile.write("new-crystal-saeldor-count = ", 0, 28);
			characterfile.write(Integer.toString(p.saeldorCount), 0, Integer.toString(p.saeldorCount).length());
			characterfile.newLine();
			characterfile.write("seren-Charge = ", 0, 15);
			characterfile.write(Integer.toString(p.serenCharge), 0, Integer.toString(p.serenCharge).length());
			characterfile.newLine();
			characterfile.write("sirenic-Mask-Charge = ", 0, 22);
			characterfile.write(Integer.toString(p.sirenicMaskCharge), 0, Integer.toString(p.sirenicMaskCharge).length());
			characterfile.newLine();
			characterfile.write("sirenic-Body-Charge = ", 0, 22);
			characterfile.write(Integer.toString(p.sirenicBodyCharge), 0, Integer.toString(p.sirenicBodyCharge).length());
			characterfile.newLine();
			characterfile.write("sirenic-Chaps-Charge = ", 0, 23);
			characterfile.write(Integer.toString(p.sirenicChapsCharge), 0, Integer.toString(p.sirenicChapsCharge).length());
			characterfile.newLine();
			characterfile.write("ethereum-Charge = ", 0, 18);
			characterfile.write(Integer.toString(p.ethereumCharge), 0, Integer.toString(p.ethereumCharge).length());
			characterfile.newLine();
			characterfile.write("craw-Charge = ", 0, 14);
			characterfile.write(Integer.toString(p.crawCharge), 0, Integer.toString(p.crawCharge).length());
			characterfile.newLine();
			characterfile.write("thamaron-Charge = ", 0, 18);
			characterfile.write(Integer.toString(p.thammaronCharge), 0, Integer.toString(p.thammaronCharge).length());
			characterfile.newLine();
			characterfile.write("viggora-Charge = ", 0, 17);
			characterfile.write(Integer.toString(p.viggoraCharge), 0, Integer.toString(p.viggoraCharge).length());
			characterfile.newLine();
			...
			characterfile.newLine();
			characterfile.write("lastDayClaimed = " + p.lastDayClaimed);
			characterfile.newLine();
			characterfile.write("dailyRewardCombo = " + p.dailyRewardCombo);
			characterfile.newLine();
			...
			characterfile.newLine();
			characterfile.write("skillPoints = ", 0, 13);
			characterfile.write(Integer.toString(p.skillPoints), 0, Integer.toString(p.skillPoints).length());
			characterfile.newLine();
			characterfile.write("pvmPoints = ", 0, 12);
			characterfile.write(Integer.toString(p.pvmPoints), 0, Integer.toString(p.pvmPoints).length());
			characterfile.newLine();
			characterfile.write("bossPoints = ", 0, 13);
			...
			characterfile.newLine();
			characterfile.write("total-xeric = ", 0, 14);
			characterfile.write(Integer.toString(p.totalXericFinished), 0, Integer.toString(p.totalXericFinished).length());
			characterfile.newLine();
			characterfile.write("total-theatre = ", 0, 16);
			characterfile.write(Integer.toString(p.totalTheatreFinished), 0, Integer.toString(p.totalTheatreFinished).length());
			characterfile.newLine();
			...
			for (Item item : p.getZulrahLostItems()) {
				if (item == null) {
					continue;
				}
			...
			for (Item item : p.getCerberusLostItems()) {
				if (item == null) {
					continue;
				}
				characterfile.write(item.getId() + "\t" + item.getAmount() + "\t");
			...
			for (Item item : p.getSkotizoLostItems()) {
				if (item == null) {
					continue;
				}
				characterfile.write(item.getId() + "\t" + item.getAmount() + "\t");
			}
			if(p.getCompletionistCape().getOverrides() != null && p.getCompletionistCape().coloursNotDefault()) {
				characterfile.newLine();
				characterfile.write("comp-cape-colours = " + p.getCompletionistCape().toString());
			}
			...
			for (int i = 0; i < Skill.length(); i++) {
				...
				characterfile.write(Integer.toString(i), 0, Integer.toString(i).length());
				characterfile.write("	", 0, 1);
				characterfile.write(Integer.toString(p.getSkills().getLevel(Skill.forId(i))), 0, Integer.toString(p.getSkills().getLevel(Skill.forId(i))).length());
				characterfile.write("	", 0, 1);
				characterfile.write(Integer.toString(p.getSkills().getExperience(Skill.forId(i))), 0, Integer.toString(p.getSkills().getExperience(Skill.forId(i))).length());
				characterfile.write("	", 0, 1);
			...
			for (Entry<Integer, Preset> entry : presets.entrySet()) {
				if (entry != null) {
					...
					for (Entry<Integer, Item> item : inventory.getItems().entrySet()) {
					...
					for (Entry<Integer, Item> item : equipment.getItems().entrySet()) {
						characterfile.write(item.getKey() + "\t" + item.getValue().getId() + "\t" + item.getValue().getAmount() + "\t");
					}
					characterfile.newLine();
				}
			...
			log.warn("Failed to save player: {}", p.playerName);
			if (Config.SERVER_STATE == ServerState.PRIVATE) {
				ioexception.printStackTrace();
			}
			return false;
		...
		//log.info("Saved player: {}", p.playerName);
		return true;
	}
